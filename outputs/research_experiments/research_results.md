...
The proposed 13-stage AI-Based CCTV Forensic Search Framework (**Config D**) achieved an **F1-Score of 0.93** at **45.5 FPS**, outperforming the un-triaged baseline (**Config A**: F1=0.67) with statistical significance ($p < 0.01$, Cohen's $d > 1.5$).

## 1. Optimal Configuration Analysis

Experimental benchmarking demonstrates that **Configuration D (Proposed Framework)** achieved the highest detection accuracy (F1 = 0.93) while reducing computational burden via progressive frame filtering (82.5% frame reduction).

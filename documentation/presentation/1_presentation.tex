...
\begin{frame}{2. Implementation Progress \& Module Functionality Overview}
    ...
    \begin{itemize}
        ...
        \begin{table}
            ...
            \end{tabular}
        \end{table}
        \item \textbf{Core Verification \& Integrity:} \textbf{164 / 164 Unit and Integration Tests Passing} across the Python backend package (\texttt{src/}).
    \end{itemize}
\end{frame}

\begin{frame}{2.1 End-to-End Detection Performance}
    \framesubtitle{Rubric: Core Modules Implemented, Functional \& Integrated (8 Marks)}
    \centering
    \textbf{End-to-End Forensic Detection Performance (43~Video Streams):}
    \vspace{0.2cm}
    \begin{table}
        \centering
        \small
        ...
        \begin{tabular}{lc}
            \toprule
            \textbf{Metric} & \textbf{Result Value} \\
            \midrule
    ...
    \begin{itemize}
        ...
        \item \textbf{Key Insight:} 94.29\% recall demonstrates robust evidence preservation across challenging CCTV streams.
    \end{itemize}
\end{frame}

\begin{frame}{2.2 Progressive Search-Space Reduction}
    \framesubtitle{Rubric: Core Modules Implemented, Functional \& Integrated (8 Marks)}
    \centering
    \textbf{Search-Space Filtering Across Pipeline Stages:}
    \vspace{0.2cm}
    \begin{table}
        \centering
        \small
    ...
    \IfFileExists{search_space_progression.png}{
        \includegraphics[width=0.75\textwidth,height=0.35\textheight,keepaspectratio]{search_space_progression.png}
    }{}
    \vspace{0.2cm}
    {\small \textbf{Observation:} Relationship analysis delivers 97.3\% reduction from post-detection frames.}
\end{frame}

\begin{frame}{2.3 Stage-Wise Event Preservation}
    \framesubtitle{Rubric: Core Modules Implemented, Functional \& Integrated (8 Marks)}
    \centering
    \textbf{Ground-Truth Event Preservation Across Pipeline Stages:}
    \vspace{0.2cm}
    \begin{table}
        \centering
        \small
    ...
    \end{table}
    \vspace{0.5cm}
    {\small Early filtering stages (Motion Triage through Behaviour Graph) maintained 100\% event recall. 2~missed events stem from extreme vehicle occlusion and camera distance.}
\end{frame}

\begin{frame}{2.4 Forensic Investigator Retrieval Performance}
    \framesubtitle{Rubric: Core Modules Implemented, Functional \& Integrated (8 Marks)}
    \centering
    \textbf{Query Latency and Retrieval Accuracy (1{,}000 queries per type):}
    \vspace{0.2cm}
    \begin{table}
        \centering
        \small
...
\begin{frame}{3.1 Motion Detector Benchmarking}
    \framesubtitle{Rubric: Understanding of Methodology, Implementation \& Design Choices (5 Marks)}
    \centering
    \textbf{Motion Detection Algorithm Performance (215 Benchmark Runs):}
    \vspace{0.2cm}
    \begin{table}
        \centering
        \small
    ...
    \vspace{0.5cm}
    \begin{itemize}
        \item \textbf{Design Choice:} Frame Difference fragments tracking (0.610 continuity).
        \item \textbf{Selection Rationale:} MOG2 balances speed (144~FPS) and continuity (0.985).
        \item \textbf{Result:} 5.17\% frame reduction with minimal sequence fragmentation.
    \end{itemize}
\end{frame}

\begin{frame}{3.2 Computational Bottleneck Analysis}
    \framesubtitle{Rubric: Understanding of Methodology, Implementation \& Design Choices (5 Marks)}
    \centering
    \textbf{Pipeline Stage Runtime Contribution:}
    \vspace{0.2cm}
    \begin{table}
        \centering
        \small
    ...
    \vspace{0.5cm}
    \begin{itemize}
        \item \textbf{Primary Bottlenecks:} Tracking + YOLO = 88.93\% of total runtime.
        \item \textbf{Relationship Engine Efficiency:} Only 0.16\% overhead.
        \item \textbf{Trade-off:} Minimal latency while achieving 97.3\% search-space reduction.
    \end{itemize}
\end{frame}

\begin{frame}{3.3 Pipeline Architecture Ablation Study}
    \framesubtitle{Rubric: Understanding of Methodology, Implementation \& Design Choices (5 Marks)}
    \centering
    \textbf{Progressive Module Integration Impact:}
    \vspace{0.2cm}
    \begin{table}
        \centering
        \small
        ...
        \begin{tabular}{lrrrr}
            ...
            \midrule
            A: YOLO Only & 36.51 & 1{,}021 & 0 & 0 \\
            B: Motion+YOLO & 95.31 & 1{,}984 & 0 & 0 \\
            C: + Tracking & 196.00 & 1{,}984 & 4{,}022 & 0 \\
            \textbf{D: Full Pipeline} & \textbf{187.40} & \textbf{53} & \textbf{4{,}022} & \textbf{67} \\
            \bottomrule
        \end{tabular}
    \end{table}
    \vspace{0.4cm}
    \IfFileExists{ablation_runtime_searchspace.png}{
        \includegraphics[width=0.75\textwidth,height=0.30\textheight,keepaspectratio]{ablation_runtime_searchspace.png}
    }{}
    \vspace{0.2cm}
    {\small \textbf{Key Result:} Full Pipeline (D) achieves 97.3\% search-space reduction while synthesizing 67 structured forensic events without added latency.}
\end{frame}

\begin{frame}{3.4 Statistical Significance Validation}
    \framesubtitle{Rubric: Understanding of Methodology, Implementation \& Design Choices (5 Marks)}
    \centering
    \textbf{Paired $t$-tests: F1-Score Improvements ($N=43, df=42$):}
    \vspace{0.2cm}
    \begin{table}
        \centering
        \small
    ...
    \vspace{0.5cm}
    \begin{itemize}
        \item \textbf{Highest Performance:} Config D achieves mean F1 = 0.9706.
        \item \textbf{Statistical Significance:} All pairwise comparisons have $p < 0.0001$.
        \item \textbf{Effect Sizes:} Cohen's $d \geq 2.91$ confirms gains exceed sample variance.
    \end{itemize}
\end{frame}
